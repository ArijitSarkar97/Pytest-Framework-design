...
// --- Type Definitions (Embedded for Reference) ---
...
export const analyzeDomAndGenerateSchema = async (
    ...
    // --- JSDOM Initialization & Global Polyfills ---
    ...
    // Polyfill globals for helper functions
    if (!global.XPathResult) {
        // @ts-ignore
    ...
    // Note: 'Element' and 'Node' might be needed if checked via instanceof, 
    // but the helpers mostly rely on types or properties. 
    // To be safe:
    if (!global.Element) {
        // @ts-ignore
    ...
    console.log(`[DOM Analysis] Found ${nodes.length} interactive elements. Processing top 50...`);

    let processedCount = 0;
    // nodes is specific NodeList, iterate manually to break early
    for (const el of Array.from(nodes)) {
        ...
        // Generate Locator
        // We cast doc to any or Document because JSDOM Document matches lib.dom.d.ts mostly
        const loc = generateLocator(el as unknown as Element, doc as unknown as Document);
